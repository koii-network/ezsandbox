...
class NamespaceWrapper {
  ...
  async auditSubmission(candidatePubkey, isValid, voterKeypair, round) {
    return await genericHandler('auditSubmission', candidatePubkey, isValid, voterKeypair, round);
  }

  async distributionListAuditSubmission(candidatePubkey, isValid, voterKeypair, round) {
    return await genericHandler('distributionListAuditSubmission', candidatePubkey, isValid, round);
  }
  ...
  async validateAndVoteOnNodes(validate, round) {
    ...
    console.log('Fetching the submissions of N - 1 round', taskAccountDataJSON.submissions[round]);
